...
public class DataView: NSView {
    ...
    public var xInterval: ClosedRange<Double> = 0...1 {
        didSet {
            needsDisplay = true
        }
    }
    ...
    public var yInterval: ClosedRange<Double> = 0...1 {
        didSet {
            needsDisplay = true
        }
    }
    ...
    public func pointAt(_ location: NSPoint) -> Point? {
        return nil
    }

    /// Converts a point from the data's coordinate system to the view's coordinate system
    public func convertDataPointToView(_ dataPoint: Point) -> CGPoint {
    ...
    public func convertViewPointToData(_ viewPoint: CGPoint) -> Point {
        let boundsXInterval = Double(bounds.minX)...Double(bounds.maxX)
        let boundsYInterval = Double(bounds.minY)...Double(bounds.maxY)
        return Point(

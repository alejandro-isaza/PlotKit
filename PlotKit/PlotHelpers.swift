...
public func plotValues<T: Sequence where T.Iterator.Element == Double>(_ values: T, hTicks: Axis.Ticks = .fit(5), vTicks: Axis.Ticks = .fit(5)) -> PlotView {
...
public func plotPoints<T: Sequence where T.Iterator.Element == Point>(_ points: T, hTicks: Axis.Ticks = .fit(5), vTicks: Axis.Ticks = .fit(5)) -> PlotView {
    let plotSize = NSSize(width: 1024, height: 400)
    let plot = PlotView(frame: NSRect(origin: CGPoint.zero, size: plotSize))
    plot.addAxis(Axis(orientation: .horizontal, ticks: hTicks))
    plot.addAxis(Axis(orientation: .vertical, ticks: vTicks))
